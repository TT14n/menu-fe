...
export function EditIngredientModal({ isOpen, onClose, onSubmit, ingredient, isMobile = false }: EditIngredientModalProps) {
  ...
  // 使用 useEffect 监听 ingredient 变化并更新表单
  useEffect(() => {
    if (ingredient) {
      setName(ingredient.name);
      setCategory(ingredient.category);
      setShelfLifeDays(ingredient.shelfLifeDays);
        ...
        <form onSubmit={handleSubmit} style={{ padding: isMobile ? '20px' : '24px' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <label style={{
              ...
              建议储存方式 <span style={{ color: '#ff4d4f' }}>*</span>
            </label>
            <div style={{
              display: 'grid',
